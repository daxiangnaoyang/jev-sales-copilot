...
function App() {
  ...
  const [reviewRecords, setReviewRecords] = useState<ReviewRecord[]>(() => {
    ...
    const initialDecision = demoDecision(demoMessages[0].text);
  ...
  const [notice, setNotice] = useState(isTauri() ? "微信会话列表不会在启动时自动导入；Bocha Jev 需在模型设置中配置并授权" : "这是浏览器演示；真实判断需在桌面 App 中调用 Bocha Jev");
  ...
  async function analyzeText(input: string, source: "manual" | "wechat" = "manual", targetConversation?: CustomerConversation): Promise<boolean> {
    ...
    if (!isTauri()) {
      setNotice("真实 Bocha Jev 调用仅在 macOS 桌面 App 中可用；本轮没有发送数据。");
      return false;
    }
    ...
    const state = nextMessages.slice(-6).map((message) => {
      const speaker = message.sender === "customer" ? "客户" : message.sender === "seller" ? "销售" : "微信 OCR（未区分说话方）";
  ...
  async function readWeChatAndAnalyze() {
    ...
    try {
      ...
      setDraft(scan.text);
      setNotice(`已识别「${title}」；点击“生成策略”后才会经已授权的 Bocha Jev 分析…`);
  ...
  async function checkBochaJev() {
    ...
    try {
      setBochaJevCheck(await invoke<string>("check_bocha_jev_provider", { apiKey: bochaJevApiKey }));
    } catch (error) {
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-area">
        <header className="topbar"><div><span className="crumb">JEV SALES COPILOT / {modeLabels[mode].label.toUpperCase()}</span><h1>{mode === "sales" ? "客户沟通副驾" : modeLabels[mode].label}</h1></div><div className="top-actions"><span className="demo-pill">{!isTauri() ? "浏览器样例" : bochaJevConsent ? "Bocha Jev 已授权" : "Bocha Jev 待授权"}</span><span className="safe-pill"><i />不自动发送</span><div className="user-avatar">象</div></div></header>
        <div className="content-scroll">
          {mode === "materials" ? <ProductMaterialsView workspace={productWorkspace} selectedOutput={selectedProductOutput} copied={copied} onUpdateField={updateProductField} onUpdateFact={updateProductFact} onAddFact={addProductFact} onImageUpload={uploadProductImage} onGenerate={generateProductPackage} onSelectOutput={(id) => setProductWorkspace((current) => ({ ...current, selectedOutputId: id }))} onCopy={copyText} /> : mode === "review" ? <ReviewView records={reviewRecords} selectedId={selectedReviewId ?? reviewRecords[0]?.id} copied={copied} onSelect={setSelectedReviewId} onMarkReviewed={markReviewReviewed} onCopy={copyText} /> : <>
            <section className="hero-row"><div><span className="overline">TODAY'S CUSTOMER MOMENT</span><h2>让每一次客户回复，<em>都有下一步</em></h2><p>Bocha Jev 负责结构化判断，Agent 再生成策略。你负责最后的判断和发送。</p></div><div className="hero-stats"><div><strong>{conversationCount}</strong><span>本次客户消息</span></div><div><strong>{Math.round(decision.confidence * 100)}%</strong><span>当前判断置信度</span></div><div><strong>{decision.commercialRisk}</strong><span>商业风险 / 9</span></div></div></section>
            <div className="workspace-grid">
              <section className="panel conversation-panel">
                ...
                <div className="message-input"><div className="input-label"><span>输入或自动读取微信聊天</span><small>{!bochaJevConsent ? "生成前需在设置中授权 Bocha Jev 接收最近对话片段" : cloudProviderSelected && shareWithProviders ? "本轮对话将发送给 Bocha Jev 和已同意的云端 Agent" : agentMode === "ollama" ? "对话发送给 Bocha Jev；策略由本机 Ollama 处理" : "本轮对话发送给 Bocha Jev；策略由本地 Demo 处理"}</small></div><textarea value={draft} onChange={(event) => setDraft(event.currentTarget.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); generateStrategy(); } }} placeholder="手动粘贴聊天内容；微信 OCR 识别后也会出现在这里……" rows={3} /><div className="input-actions"><button className="text-button" onClick={resetDemo}><Icon name="refresh" size={13} />{selectedConversation?.profile.id === demoCustomer.id ? "重置案例" : "清空草稿"}</button><span>Enter 生成 · Shift + Enter 换行</span><button className="primary-button small-button" onClick={generateStrategy} disabled={!draft.trim() || wechatBusy || analysisBusy}><Icon name="spark" size={14} />{analysisBusy ? "处理中…" : "生成策略"}</button></div></div>
              </section>
